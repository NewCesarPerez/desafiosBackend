...
//     const productById = await cartServices.getCartsById(idProductToCompare);
...
//     await cartServices.updateCartById(cartById.id, cartById);
...
export const postProductInCartById = async (req, res) => {
  ...
  if (idProductToCompare && cartIdToUse) {
    const newCart = await cartServices.addProductToCart(cartIdToUse, idProductToCompare);
 
    if(!newCart) return res.status(400).json({'message':'No se a agregado el producto al carrito'});
...
export const deleteProductFromCartById = async (req, res) => {
  ...
  if (idProductToCompare && cartIdToUse) {
    const newCart=await cartServices.deleteProductfromCart(cartIdToUse, idProductToCompare);
    if(!newCart) return res.status(400).json({'message':'No se ha agregado el producto al carrito'});
